...
class ResizeShortSize:
    def __init__(self, short_size, resize_text_polys=True):
        """
        :param short_size: 目标短边大小，确保图像的短边 >= short_size
        :param resize_text_polys: 是否调整文本框坐标
        """
    ...
    def __call__(self, data: dict) -> dict:
        """
        对 img 和 snow_img 进行缩放，并同步调整文本框坐标
        :param data: {'img':, 'snow_img':, 'text_polys':, 'texts':, 'ignore_tags':}
        :return:
        """
        im = data['img']
        snow_im = data.get('snow_img', None)  # 兼容没有 snow_img 的情况
        text_polys = data['text_polys']
        ...
        if short_edge < self.short_size:
            ...
            if snow_im is not None:
                snow_im = cv2.resize(snow_im, dsize=None, fx=scale, fy=scale)
