...
class EastRandomCropData():
    ...
    def __call__(self, data: dict) -> dict:
        im = data['img']
        snow_im = data.get('snow_img', None)
        ...
        def crop_and_resize(image):
            if self.keep_ratio:
                if len(image.shape) == 3:
                    padimg = np.zeros((self.size[1], self.size[0], image.shape[2]), image.dtype)
                else:
                    padimg = np.zeros((self.size[1], self.size[0]), image.dtype)
                padimg[:h, :w] = cv2.resize(image[crop_y:crop_y + crop_h, crop_x:crop_x + crop_w], (w, h))
                return padimg
            else:
                return cv2.resize(image[crop_y:crop_y + crop_h, crop_x:crop_x + crop_w], tuple(self.size))

        data['img'] = crop_and_resize(im)
        if snow_im is not None:
            data['snow_img'] = crop_and_resize(snow_im)
