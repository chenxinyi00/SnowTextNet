...
def get_datalist(train_data_path):
    """
    获取训练和验证的数据list，确保返回成对的（雪图，去雪图，标注）路径
    :param train_data_path: 训练的dataset文件列表，每个文件内以如下格式存储：
        'path/to/snow_img.jpg\tpath/to/clear_img.jpg\tpath/to/label.txt'
    :return: [(snow_img_path, clear_img_path, label_path), ...]
    """
    train_data = []
    for p in train_data_path:
        with open(p, 'r', encoding='utf-8') as f:
            for line in f.readlines():
                ...
                if len(line) == 3:  # 确保有三列数据
                    snow_img_path = pathlib.Path(line[0].strip())
                    img_path = pathlib.Path(line[1].strip())
                    label_path = pathlib.Path(line[2].strip())

                    # 检查文件是否存在且不为空
                    if (
                        snow_img_path.exists() and snow_img_path.stat().st_size > 0 and
                        img_path.exists() and img_path.stat().st_size > 0 and
                        label_path.exists() and label_path.stat().st_size > 0
                    ):
                        train_data.append((str(snow_img_path), str(img_path), str(label_path)))
    return train_data
