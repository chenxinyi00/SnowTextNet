...
class Text(data.Dataset):
    def __init__(self, path, train=False, size=256, format='.jpg', rand_inpaint=False, rand_augment=None):
        ...
        if self.train:
            self.haze_imgs_dir = os.listdir(os.path.join(path, 'img'))
        else:
            self.haze_imgs_dir = os.listdir(os.path.join(path, 'img'))

        print('======>total number for training:', len(self.haze_imgs_dir))
        self.haze_imgs = [os.path.join(path, 'img', img) for img in self.haze_imgs_dir]
        self.clear_dir = os.path.join(path, 'gt')
        self.guidance_dir = os.path.join('/data1/cxy/Text/guide')
    ...
    def __getitem__(self, index):
        ...
        id = img.split('/')[-1].split(".")[0]
        clear_name = id + '.' + self.format
        guidance_name = id + '.' + self.format

        clear = Image.open(os.path.join(self.clear_dir, clear_name))
        guidance = Image.open(os.path.join(self.guidance_dir, guidance_name))

        clear = tfs.CenterCrop(haze.size[::-1])(clear)
        guidance = tfs.CenterCrop(haze.size[::-1])(guidance)

        if not isinstance(self.size, str) and self.train:
            i, j, h, w = tfs.RandomCrop.get_params(haze, output_size=(self.size, self.size))
            haze = FF.crop(haze, i, j, h, w)
            clear = FF.crop(clear, i, j, h, w)
            guidance = FF.crop(guidance, i, j, h, w)

        haze, clear, guidance = self.augData(haze.convert("RGB"), clear.convert("RGB"), guidance.convert("RGB"))
        return haze, clear, guidance, id

    def augData(self, data, target, guidance):
        if self.train:
            rand_hor = random.randint(0, 1)
            rand_rot = random.randint(0, 3)
            data = tfs.RandomHorizontalFlip(rand_hor)(data)
            target = tfs.RandomHorizontalFlip(rand_hor)(target)
            guidance = tfs.RandomHorizontalFlip(rand_hor)(guidance)
            if rand_rot:
                data = FF.rotate(data, 90 * rand_rot)
                target = FF.rotate(target, 90 * rand_rot)
                guidance = FF.rotate(guidance, 90 * rand_rot)

        data = tfs.ToTensor()(data)
        target = tfs.ToTensor()(target)
        guidance = tfs.ToTensor()(guidance)

        return data, target, guidance
...
# class Text1(data.Dataset):
#     def __init__(self, path, train=False, size=256, format='.jpg', rand_inpaint=False, rand_augment=None):
#         super(Text1, self).__init__()
#         self.size = size
#         self.rand_augment = rand_augment
#         self.rand_inpaint = rand_inpaint
#         self.InpaintSize = 64
...
class Text1(data.Dataset):
    def __init__(self, path, train=False, size=256, format='.jpg', rand_inpaint=False, rand_augment=None):
        ...
        self.haze_imgs = [os.path.join(path, 'img', img) for img in self.haze_imgs_dir]
        self.clear_dir = os.path.join(path, 'gt')
        self.guidance_dir = os.path.join(path, 'guide')
...
class TextTD(data.Dataset):
    def __init__(self, path, train=False, size=256, format='.JPG', rand_inpaint=False, rand_augment=None):
        super(TextTD, self).__init__()
        self.size = size
        self.rand_augment = rand_augment
        self.rand_inpaint = rand_inpaint
        self.InpaintSize = 64
