    ...
    def __init__(self, config, model, criterion):
        ...
        if self.tensorboard_enable and config['local_rank'] == 0:
            ...
            try:
                ...
                dummy_img = torch.zeros(1, in_channels, 640, 640).to(self.device)
                dummy_snow_img = torch.zeros(1, in_channels, 640, 640).to(self.device)
                self.writer.add_graph(self.model, (dummy_img, dummy_snow_img))  # 修改点
                torch.cuda.empty_cache()
            except:
                import traceback
                self.logger.error(traceback.format_exc())
                self.logger.warning('add graph to tensorboard failed')

        # 分布式训练
        if torch.cuda.device_count() > 1:
            local_rank = config['local_rank']
            self.model = torch.nn.parallel.DistributedDataParallel(
                self.model, device_ids=[local_rank], output_device=local_rank, broadcast_buffers=False,
